...
1. **先查注册表**：人物与账号映射在 `config/person_crawl.yaml`。执行前用 `--list` 确认 `person_id`、X handle、微博 UID。
...
5. **声明你在用什么**：例如「调用 spider x runner → parse_twitter_jsonl → knowledge/people/desmond_shum/x/」。
...
| 预览 spider 目标 | `python scripts/crawl_person_sources.py desmond_shum --dry-run` |
| 爬取并导入 X+微博 | `python scripts/crawl_person_sources.py desmond_shum` |
| 只爬 X | `python scripts/crawl_person_sources.py desmond_shum --platform x` |
| 只爬微博 | `python scripts/crawl_person_sources.py desmond_shum --platform weibo` |
| 已有爬取结果，只导入 | `python scripts/crawl_person_sources.py desmond_shum --skip-crawl` |
| 导入后重建 RAG | 加 `--ingest-rag` |

## 新增人物

1. 在 `config/person_crawl.yaml` 的 `people` 下追加条目（`person_id` 与 `knowledge/people/` 目录名一致）：
...
2. 确保 `knowledge/people/new_person/{x,weibo}/` 存在（脚本会自动创建）。
...
python scripts/parse_twitter_jsonl.py /path/to/tweets.jsonl \
  --command export-md --person-name desmond_shum --display-name "Desmond Shum" \
...
python scripts/parse_weibo_json.py /path/to/uid.json \
  --command export-md --person-name desmond_shum --display-name "显示名" \
  --originals-only --min-length 80

# RAG
python -m rag.ingest --person-name desmond_shum --source-kind x --embedding-provider keyword
python -m rag.ingest --person-name desmond_shum --source-kind weibo --embedding-provider keyword
```

## 相关文件

- `person_crawl_agent/` — 配置加载、spider 桥接、定位输出、导入流水线
- `config/person_crawl.yaml` — 人物 ↔ 平台账号注册表
