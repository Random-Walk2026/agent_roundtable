...
│   ├── loader.py            # 读取 config/domain_experts/、persona_inspired/、councils/ 的 YAML
...
例如 `config/domain_experts/macro_economist.yaml` 代表“宏观经济与制度专家”。
...
这里放运行配置。
目前最重要的是 `config/agent_llms.json`，它负责每个 Agent 用什么模型。
